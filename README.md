...
Click the bracket `[ ]` in front of each code block sequentially to run the code. Make sure to read the descriptions carefully. If you get an error importing the pet dataset, change the code to `dataset, info = tfds.load('oxford_iiit_pet:4.*.*', with_info=True)`.
